    ...
    for fault_name in sorted(faults):
        ...
        if not should_drop_fault(most_severe):
            ...
            row = {
                ...
                "Rakuten Alarm Code": rakuten_alarm_code(most_severe),
...
def rakuten_alarm_code(fault):
    return "1AML-NET-UHN05-03-" + rakuten_oss_alarm_id(fault)

RAKUTEN_OSS_ID_CURRENT = 40000
def rakuten_oss_alarm_id(fault):
    # Rakuten has a silly alarm id system of reserved 6 digit numbers that must be unique per alarm.
